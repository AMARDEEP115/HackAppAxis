import Navbar from "../component/navbar";
import { Box, Heading } from "@chakra-ui/react";
import React from "react";
import tutorialStyle from "../AllCss/Tutorial.module.css";
import Footer from "../component/Footer";
import SingleTutorial from "@/component/Singletutorial";

const Tutorial = () => {
  const [dash,setDash]=React.useState(true);
  const [react,setReact]=React.useState(false);
  const [mongo,setMongo]=React.useState(false);
  const [express,setExpress]=React.useState(false);
  const [node,setNode]=React.useState(false);

  const handleReact=()=>{
    setDash(false);
    setReact(true);
    setMongo(false);
    setExpress(false);
    setNode(false);
  };
  const handleMongo=()=>{
    setDash(false);
    setReact(false);
    setMongo(true);
    setExpress(false);
    setNode(false);
  };
  const handleExpress=()=>{
    setDash(false);
    setReact(false);
    setMongo(false);
    setExpress(true);
    setNode(false);
  };
  const handleNode=()=>{
    setDash(false);
    setReact(false);
    setMongo(false);
    setExpress(false);
    setNode(true);
  };
  const handleBack=()=>{
    setDash(true);
    setReact(false);
    setMongo(false);
    setExpress(false);
    setNode(false);
  };

  console.log("react",react,"mongo",mongo,'express',express,'node',node)
  const playerRef = React.useRef<any>();

  let ReactInfo = [
    {time:73,title:'Differentiate between Virtual DOM and Real DOM'}, {time:183,title:'What is React?'}, {time:272,title:'What is the meaning of Virtual DOM?'}, {time:351,title:'What are some of the important features of React?'}, {time:444,title:'What is the meaning of JSX?'}, {time:521,title:'Can browsers read a JSX file?'}, {time:593,title:'Why is React widely used today?'}, {time:708,title:'Are there any disadvantages of using React?'}, {time:812,title:'Differentiate between Angular and React'}, {time:884,title:'What is the meaning of the component-based-architecture of React?'}, {time:957,title:'How does rendering work in React?'}, {time:1017,title:'What are states in React?'}, {time:1054,title:'What are props in React?'}, {time:1117,title:'What is the use of an arrow function in React?'},
    {time:1187,title:'What is a higher-order-component in React?'}, {time:1256,title:'What is the meaning of create-react-app in React?'}, {time:1313,title:'What are some of the advantages of using create-react-app in React?'}, {time:1382,title:'What is the meaning of Redux?'}, {time:1461,title:'What is the difference between props and states?'}, {time:1523,title:'What are the three phases of a component life cycle in React?'}, {time:1607,title:'What are events in React?'}, {time:1667,title:'How are events created in React?'}, {time:1725,title:'How is routing in React different from conventional routing?'}, {time:1825,title:'Differentiate between Flux and Redux in React.'}, {time:1920,title:"Can AJAX be used with React?"}, {time:1988,title:'What is the meaning of synthetic events in React?'},
    {time:2047,title:'What are stateful components in React?'}, {time:2097,title:'What are refs in React?'}, {time:2164,title:'What are controlled components in React?'}, {time:2222,title:'Why is a router required in React?'},
  ];

  let NodeInfo = [
    {time:37,title:'What is Node JS?'},
    {time:87,title:'What is the difference between a Node JS and Javascript?'},
    {time:183,title:'Can you briefly explain the working of Node JS?'},
    {time:224,title:'Where is Node JS used?'},
    {time:280,title:'What is the difference between Node JS and Angular?'},
    {time:397,title:'Why is Node JS single-threaded?'},
    {time:448,title:'What are the different API functions supported by Node JS?'},
    {time:499,title:'What is the difference between synchronous and asynchronous functions?'},
    {time:596,title:'What is the meaning of control flow function?'},
    {time:641,title:'Why is Node JS so popular these days?'}
  ]

  let ExpressInfo = [
    {time:33, title:'What is Express JS?'},
    {time:153, title:'What are the Express JS features?'},
    {time:426, title:'How to install Express JS?'},
    {time:524, title:'What is Scaffolding in Express JS?'},
    {time:677, title:'How to serve static files in Express JS?'},
    {time:761, title:'What is Routing in Express JS?'},
    {time:898, title:'What is dynamic routing in Express JS?'},
    {time:992, title:'What is middelware in Express JS?'},
    {time:1153, title:'Which templates Engines does support express?'},
    {time:1191, title:'What is HTTP methods?'},
  ]

  let MongoInfo = [
    {time:17,title:'What is NoSQL?'},
    {time:50, title:'When to use NoSQL?'},
    {time:82, title:'When to use SQL?'},
    {time:126, title:'How does NoSQL database management system budget memory?'},
    {time:156, title:'What is eventual consistency in NoSQL Stores?'},
    {time:183, title:'What is Namespace in MongoDB?'},
    {time:204, title:'What is the syntax to create a collection and to drop a collection in MongoDB?'},
    {time:237, title:'Mention what is ObjectID composed of?'},
    {time:276, title:'In a single MongoDB document what is the store capacity?'},
    {time:294, title:'What is the alternate to store more than 16MB data in MongoDB document?'},
  ]


  return (
    <Box pb="20px">
      <Navbar />
      <button className={tutorialStyle.backBtn} onClick={()=>handleBack()}>Back To Tutorials</button>
      {dash && <div className={tutorialStyle.dash}>
        <div className={tutorialStyle.dashDiv} onClick={()=>handleReact()}>
          <img src="data:image/png;base64," alt="react" />
          <div className={tutorialStyle.dashDivIn}>
            <h1>REACT: </h1>
            <h2>React is a free and open-source front-end JavaScript library for building user interfaces based on UI components. It is maintained by Meta and a community of individual developers and companies.</h2>
          </div>
        </div>
        <div className={tutorialStyle.dashDiv} onClick={()=>handleMongo()}>
          <img src="data:image/png;base64," alt="mongo"/>
          <div className={tutorialStyle.dashDivIn}>
            <h1>MONGO : </h1>
            <h2>MongoDB is a source-available cross-platform document-oriented database program. Classified as a NoSQL database program, MongoDB uses JSON-like documents with optional schemas. MongoDB is developed by MongoDB Inc. and licensed under the Server Side Public License which is deemed non-free</h2>
          </div>
        </div>
        <div className={tutorialStyle.dashDiv} onClick={()=>handleExpress()}>
          <img src="data:image/png;base64," alt="express"/>
          <div className={tutorialStyle.dashDivIn}>
            <h1>EXPRESS :</h1>
            <h2>Express.js, or simply Express, is a back end web application framework for building RESTful APIs with Node.js, released as free and open-source software under the MIT License. It is designed for building web applications and APIs.</h2>
          </div>
        </div>
        <div className={tutorialStyle.dashDiv} onClick={()=>handleNode()}>
          <img src="data:image/png;base64," alt="node" />
          <div className={tutorialStyle.dashDivIn}>
            <h1>NodeJS :</h1>
            <h2>Node.js is a cross-platform, open-source server environment that can run on Windows, Linux, Unix, macOS, and more. Node.js is a back-end JavaScript runtime environment, runs on the V8 JavaScript Engine, and executes JavaScript code outside a web browser</h2>
          </div>
        </div>
      </div>}
      {react && <SingleTutorial tit={"React"} info={ReactInfo} Url={'https://www.youtube.com/watch?v=5UKe5BMnJgk&t=595s'}/>}
      {mongo && <SingleTutorial tit={"Mongo"} info={MongoInfo} Url={'https://www.youtube.com/watch?v=geB_GAWwDUk&t=436s'}/>}
      {express && <SingleTutorial tit={"Express"} info={ExpressInfo} Url={'https://www.youtube.com/watch?v=vS4nOHMYEgY&t=1242s'}/>}
      {node && <SingleTutorial tit={"Node JS"} info={NodeInfo} Url={'https://www.youtube.com/watch?v=7npkRw7gMRA&t=644s'}/>}
      <Footer/>
    </Box>
  );
};

export default Tutorial;
